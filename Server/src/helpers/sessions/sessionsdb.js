...
function joinSession(sessionId, user, callback) {
    ...
    isUserInAnySession(user.userId, function(err, inAnySession) {
        if (err) {
            ...
            callback(`User ${user.userId} is already in another session`);
        } else {
                ...
                if (err || !result) {
                    ...
                    result.populate({
                        ...
                        if (err) {
                            ...
                            callback(`Book could not be populated after finding the Session ${result._id}`);
                        } else {
                                    ...
                                    if (err || !result) {
                                        ...
                                        result.populate({
                                            ...
                                            if (err) {
                                                ...
                                                callback(`Book could not be populated after updating Session ${result._id}`);
                                            } else {
                                                callback(err, result);
                                                if (webSockets) {
